...
document.getElementById("modal-btn").addEventListener("click", () => {
	...
	if (status.includes("false")) {
		...
		fetch("http://localhost:3000/QandA/add/question", {
			method: "POST",
			headers: {
...
document.getElementById("quesSearch").addEventListener("input", (event) => {
	...
	fetch("http://localhost:3000/QandA/search?q=" + query)
		.then((res) => res.json())
		.then((res) => {
			...
			for (let i = 0; i < questions.length; i++) {
				if (
					questions[i].QUESTION_TITLE.toLowerCase() !==
					"test question"
				) {
					content =
						content +
...
function postAnswer(event) {
	...
	//fetch call
		...
		document.getElementById("answer-modal-btn").value = "Please Wait...";
		fetch("http://localhost:3000/QandA/add/answer", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
