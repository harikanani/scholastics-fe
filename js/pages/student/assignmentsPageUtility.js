...
const getAssignments = async () => {
	...
	const response = await fetch(
		`http://localhost:3000/assignments/student/allAssignments`,
		{
			method: "GET",
			headers: new Headers({
                    ...
                    ${
                      ...
                      <input onclick=fileUploadHandler(event) data-courseId=${courseId} onclick=submitButtonHandler(event)
                       ...
                       id="submit-btn" style="color: white; width: 60%; border-color: #00564d;" />`
							: !answerScriptLink
							? `<div style="color: red">Deadline has passed </div>`
							: ``
					}
                    </div>
                </div>
...
const fileUploadHandler = (event) => {
	...
	fetch("http://localhost:3000/assignments/submitAssignment", {
		method: "POST",
		headers: new Headers({
