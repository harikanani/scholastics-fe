	...
	.then(({ courseMessages }) => {
		...
		document
			...
			.firstChild.setAttribute(
				"src",
				`https://ui-avatars.com/api/?name=${localStorage.getItem(
					"name",
				)}`,
			);
	})
...
window.addEventListener(
	...
	function (event) {
		if (window.innerWidth <= 990) {
			document.getElementById("button-navigation").style.display =
				"block";
		} else {
