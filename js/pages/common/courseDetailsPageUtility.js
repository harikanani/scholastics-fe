...
const getLatestAnnouncementStudent = (courseId, courseMessages) => {
	...
	var sched_date =
		...
		moment(courseMessage.value.COURSE_SCHEDULE_POSTED_ON).format(
			"MMM Do YY",
		);
...
const getCourseDetailsAndPopulateUI = (courseId) => {
	...
	fetch(
		`http://localhost:3000/courses/getCourseDetails?courseId=${courseId}`,
		{
			method: "GET",
			headers: {
		...
		.then((response) => {
			if (response) {
				...
				response.courseDetails.ENROLLED_STUDENTS.forEach(
					(card, index) => {
            ...
            ${
				...
				!isTeacher &&
					document.getElementById("th-remove-student").remove();
		...
		.then((response) => {
			if (response.courseMessages) {
				document.getElementById("view-message").innerHTML =
					getLatestAnnouncementStudent(
						courseId,
						response.courseMessages,
					)[1] || "No new Messages";
