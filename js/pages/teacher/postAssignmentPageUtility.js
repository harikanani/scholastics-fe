...
fileInput.onchange = (e) => {
	...
	if (selectedFile) {
		document.getElementById("ques-file-upload-btn").value =
			"1 File Selected";
	} else {
		e.preventDefault();
		Swal.fire({
...
document
	...
	.addEventListener("click", () => {
		...
		const assignDeadline = document.getElementById(
			"assignment-deadline",
		).value;
		...
		if (quesTitle.length < 1) {
			document.getElementById("assign-ques-title").style.borderColor =
				"red";
			document.getElementById("assign-ques-title").value = "";
		...
		if (assignDeadline.length < 1) {
			document.getElementById("assignment-deadline").style.borderColor =
				"red";
			document.getElementById("assignment-deadline").value = "";
		...
		if (status.includes("false")) {
			...
			fetch("http://localhost:3000/assignments/addAssignment", {
				method: "POST",
				headers: new Headers({
					Authorization: token,
				}),
				...
				.then((res) => {
					if (res.message) {
						...
						document.getElementById(
							"teacher-assign-post-btn",
						).innerText = "Post Assignment";
						resetInputValues();
					} else {
						Swal.fire({
							icon: "error",
...
const fetchPostedAssignments = () => {
	fetch("http://localhost:3000/assignments/teacher/allAssignments", {
		method: "GET",
		headers: {
			"Content-Type": "application/json",
			Authorization: `${token}`,
		...
		.then((res) => {
			...
			document.getElementById("posted-assignments-details").innerHTML =
				output;
		})
