				...
				response.courseDetails.ENROLLED_STUDENTS.forEach((card) => {
					if (card.ASSIGNMENT_SUBMISSION_FILE_ID?.trim()?.length) {
						...
						output =
                            ...
                            <td>${student_name}</td>
                            <td>${
								card.ASSIGNMENT_SUBMISSION_LINK
									? `<a target="_blank" href="${submn}"><i class="fas fa-link"></i></a>`
									: `-`
							} </td>
                            <td>${student_score}</td>
...
const postMarks = (event) => {
	...
	if (score < 0) {
		document.getElementsByClassName("mark-input")[0].style.borderColor =
			"red";
		...
		document.getElementsByClassName("btn update-btn-modal")[0].value =
			"Loading...";
		fetch("http://localhost:3000/assignments/gradeAssignment", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				Authorization: `${token}`,
