	...
	.addEventListener("click", () => {
		...
		if (status.includes("false")) {
			...
			document.getElementById("office-hour-submit-btn").innerText =
				"Please wait...";
			fetch("http://localhost:3000/facultyUtility/updateOfficeHour", {
				method: "POST",
				headers: new Headers({
					"Content-Type": "application/json",
					Authorization: token,
				...
				.then((res) => {
					if (res.message) {
						...
						document.getElementById(
							"office-hour-submit-btn",
						).innerText = "Post Office Hour";
		...
		if (response) {
			response.courses.forEach((card) => {
				if (card.OFFICE_HOUR_DAY) {
					...
					const cardCategoryTextNode =
						document.createTextNode(course_id);
					...
					const cardTitleTextNode =
						document.createTextNode(course_name);
					...
					cardTitleH6.setAttribute(
						"style",
						"padding-left:16px; font-size: 16px; margin:5px",
					);
					...
					setInterval(() => {
						...
						if (day === card?.OFFICE_HOUR_DAY?.trim()) {
							...
							const endHour = Number(
								card.OFFICE_HOUR_END_TIME.split(":")[0],
							);
							const endMinute = Number(
								card.OFFICE_HOUR_END_TIME.split(":")[1],
							);
							const currentHour = date.getHours();
							const currentMinute = date.getMinutes();
							const startMinuteConv =
								startMinute + startHour * 60;
							const endMinuteConv = endMinute + endHour * 60;
							const currentMinuteConv =
